...
- With `show_chat_dates`, chat rows render the last-message timestamp in local time with a 24-hour clock. `chatLastMessageTimestamp` omits the current year but includes it for older rows; the aligned column is 16 cells wide.
...
- **Focus Tracking & Dashboard Mode**:
	...
	- The 3-second message polling for the active chat is automatically paused when the window is blurred or no conversation is selected.
	- `leaveActiveConversation()` in `dashboard.go` clears chat/channel identity and transient conversation state while retaining sidebar data and filters.
	- Pressing `q` or `Esc` in normal mode leaves the current conversation and shows the dashboard. A second `q` from the dashboard quits; `Ctrl+C` always quits immediately.
	- `rebuildChatList()` must preserve `SelectedIndex == -1`, and navigating to a chat/channel resumes polling and pulls the latest messages.
